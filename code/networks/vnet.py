...
class VNet(nn.Module):
    ...
    def forward(self, input, turnoff_drop=False, need_fp=False):
        ...
        if need_fp:
            ...
            return outs.chunk(2 + len(self.pert)), perted_fus[0]
...
class PertDropout(nn.Module):
    def __init__(self, p=0.5, type="dropout"):
        ...
        print("-" * 25, f"Info: Using 3D dropout with {top}", "-" * 25)
        ...
        self.dropouts = [dropout_type[type](top).cuda()]

        self.len = len(self.dropouts)

    def __len__(self):
        # print("The len of dropouts:", self.len)
        return self.len
...
if __name__ == "__main__":
    ...
    net = VNet(
        n_channels=1, n_classes=2, normalization="batchnorm", has_dropout=True, pert_gap=0.5,
        pert_type='dropout'
    )
    img_x = torch.randn(4, 1, 112, 112, 80)
    img_u_w = torch.randn(4, 1, 112, 112, 80)
    [pred_x, pred_u_w, pred_u_w_strong], perted_fus_t = net(
    ...
    print(pred_u_s1.type)

    pred_u_w_strong.softmax(dim=1)

    # pred_x, pred_u_w, pred_u_w_strong, perted_fus = net(
    #     torch.cat((img_x, img_u_w)),
    #     need_fp=True
    # )
