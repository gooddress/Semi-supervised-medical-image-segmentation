...
from utils.losses import dice_loss, kl_divergence
...
parser.add_argument("--label_num", type=int, default=4, help="label num")
parser.add_argument(
...
parser.add_argument('--distill_loss', type=str, default='CE', help='feature pertubation types')
...
snapshot_path = "../model/" + args.exp + "/" + f"{args.distill_loss}_{args.temperature}" + "/" 
...
if __name__ == "__main__":
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, (
            ...
            [pred_x, pred_u_w, pred_u_w_strong], perted_u_w_strong = net(
            ...
            [pred_u_s1, pred_u_s2, pred_u_s2_strong], perted_u_s2_strong = net(
                torch.cat((img_u_s1, img_u_s2)),
                need_fp=True
            )
            ...
            # perted_u_w_strong to supervise perted_u_s2_strong:CE
            if args.distill_loss == 'MSE':
                loss_bottle = F.mse_loss(perted_u_w_strong[0], perted_u_s2_strong[0])
            elif args.distill_loss == 'KL':
                loss_bottle = (
                    F.kl_div(F.log_softmax(perted_u_w_strong[0] / args.temperature, dim=1), 
                    F.softmax(perted_u_s2_strong[0].detach() / args.temperature, dim=1)) +
                    F.kl_div(F.log_softmax(perted_u_s2_strong[0] / args.temperature, dim=1), 
                    F.softmax(perted_u_w_strong[0].detach() / args.temperature, dim=1))
                ) / 2.0
            elif args.distill_loss =='CE':
                loss_bottle = (F.log_softmax(perted_u_w_strong[0] / args.temperature, dim=1) * 
                               F.softmax(perted_u_s2_strong[0].detach() / args.temperature, dim=1)).mean()
            ...
            loss_u_w_kd = (1 - eta) * (
                     loss_strong_dec_w_t + loss_strong_dec_w_s2_t
                      ) / 2.0 + eta * loss_strong_dec_w_s2

            loss = (loss_x + loss_u_s1 * 0.5 + loss_u_s2 * 0.5 + loss_u_w_kd + loss_bottle) / 4.0
#            print(loss)
#            print(loss.shape)
        ...
        if (
            ...
            with torch.no_grad():
                ...
                logging.info('iteration %d : dice : %f jc:%f hd:%f asd:%f ' %(iter_num, dice.item(), jc.item(), hd.item(), asd.item()))

                if dice > pervious_bset_dice and dice < 1.0 :
